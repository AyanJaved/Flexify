...
def sync_metrics_update(context):
    ...
    if workout_completed and not st.session_state.get("last_notified_workout_complete", False):
        ...
        # All target sets are done -- actually end the session instead of
        # just notifying. This flips workout_started off (so the sidebar
        # goes back to the "Start Workout" state) and forces a full-app
        # rerun so the webrtc_streamer branch is skipped and the camera
        # is torn down. scope="app" is required here because this function
        # runs inside an @st.fragment -- a plain st.rerun() would only
        # re-run the fragment, not the outer script that owns the sidebar
        # and the webrtc_streamer call.
