...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        # Process every Nth frame through the pose model to avoid falling
        # behind real-time on CPU. Frames in between are still returned
        # (unprocessed) so the video preview itself stays smooth.
        self._frame_count = 0
        self._process_every_n = 2

        model_path = os.path.join(os.getcwd(), "ml_models", "pose_landmarker_lite.task")
    ...
    def recv(self, frame):
        ...
        # Skip pose inference on some frames to avoid falling behind
        # real-time. The video preview still updates every frame; only
        # the (expensive) landmark detection is throttled.
        ...
        try:
            ...
            # Without this, an exception here is silently swallowed by the
            # streamlit-webrtc background thread and the video just freezes
            # with no visible error anywhere. Printing it to the terminal
            # is the only way to see what actually went wrong.
            print("=" * 60)
            print("EXCEPTION INSIDE VideoProcessorClass.recv():")
            traceback.print_exc()
            print("=" * 60)
